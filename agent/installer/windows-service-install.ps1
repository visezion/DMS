...
function Assert-AgentPackageLayout {
  param(
  ...
  $hasSelfContainedMarker = $runtimeProbeFiles | Where-Object { Test-Path (Join-Path $RootPath $_) -PathType Leaf }
  if ($hasSelfContainedMarker.Count -gt 0) {
    $requiredFiles += $runtimeProbeFiles
  }

  $missing = @()
  foreach ($name in $requiredFiles | Select-Object -Unique) {
    if (-not (Test-Path -Path (Join-Path $RootPath $name) -PathType Leaf)) {
      $missing += $name
    }
  }

  if ($missing.Count -gt 0) {
    throw "$Label is incomplete. Missing files: $($missing -join ', ')"
  }
}
...
function Wait-ForServiceRunning {
  ...
  $service = Get-Service -Name $Name -ErrorAction SilentlyContinue
  if ($null -eq $service) {
    throw "Service $Name was not found after installation."
  }

  throw "Service $Name did not reach Running state. Current status: $($service.Status)"
}

function Invoke-AgentDiagnosticsOrThrow {
  param(
    [Parameter(Mandatory = $true)][string]$ExecutablePath
  )

  $output = & $ExecutablePath --diagnostics 2>&1
  $exitCode = $LASTEXITCODE
  if ($exitCode -ne 0) {
    $message = if ($output) { ($output | Out-String).Trim() } else { "no diagnostics output" }
    throw "Agent diagnostics failed before service registration. $message"
  }
}
...
Assert-AgentPackageLayout -RootPath $SourceDir -Label "Agent source package"
...
Assert-AgentPackageLayout -RootPath $InstallDir -Label "Installed agent package"
...
Invoke-AgentDiagnosticsOrThrow -ExecutablePath $ExePath
...
Wait-ForServiceRunning -Name $ServiceName -TimeoutSeconds 20
