...
internal static class ProcessRunner
{
    ...
    public static string EscapeForSchtasksTaskRun(string command)
    {
        ...
        // Normalize any quote-escapes so /TR receives exactly one backslash before embedded quotes.
        string normalized = Regex.Replace(command, "\\\\+\"", "\\\"");

        // Keep escaping idempotent to support payloads that already escaped embedded quotes.
...
""";
    ...
    private static async Task<(bool Compliant, string Message)> ApplyScheduledTaskAsync(JsonElement config, bool enforce, CancellationToken cancellationToken)
    {
        ...
        if (enforce)
        {
            ...
            {
                string errorText = string.IsNullOrWhiteSpace(createResult.StdErr)
                    ? createResult.StdOut
                    : createResult.StdErr;
                string diagnostic = string.IsNullOrWhiteSpace(errorText)
                    ? $"schtasks exited with code {createResult.ExitCode}"
                    : errorText.Trim();
                return (false, $"scheduled task create failed: {diagnostic}");
            }
        }
        ...
        CancellationToken cancellationToken)
    {
        ...
        string escapedTaskRunCommand = ProcessRunner.EscapeForSchtasksTaskRun(taskRunCommand);
        try
        {
            var create = await ProcessRunner.RunAsync(
                "schtasks",
                $"/Create /TN \"{taskName}\" /SC ONCE /ST 00:00 /RU SYSTEM /RL HIGHEST /TR \"{escapedTaskRunCommand}\" /F",
                cancellationToken);
...
public sealed class ScreenCaptureHandler : IJobHandler
{
    private const int SmXVirtualScreen = 76;
    private const int SmYVirtualScreen = 77;
    private const int SmCxVirtualScreen = 78;
    private const int SmCyVirtualScreen = 79;
    private const int InteractiveHelperTimeoutSeconds = 20;

    public string JobType => "capture_screen";

    public Task<(string Status, int ExitCode, object? Result)> ExecuteAsync(CommandEnvelopeDto envelope, CancellationToken cancellationToken)
    {
        if (!RuntimeInformation.IsOSPlatform(OSPlatform.Windows))
        {
            return Task.FromResult(("failed", 1, (object?)new { error = "capture_screen is currently supported on Windows endpoints only" }));
        }

        int maxDimension = ReadInt(envelope.Payload, "max_dimension", 1366, 640, 3840);
        int jpegQuality = ReadInt(envelope.Payload, "jpeg_quality", 60, 25, 95);
        string sessionId = envelope.Payload.TryGetValue("session_id", out var sessionObj) ? sessionObj?.ToString() ?? string.Empty : string.Empty;

        try
        {
            (string base64, int byteSize) = CaptureAsJpegBase64(maxDimension, jpegQuality);
            return Task.FromResult(("success", 0, (object?)new
            {
                captured_at_utc = DateTimeOffset.UtcNow.ToString("O"),
                image_base64 = base64,
        ...
        catch (Exception ex)
        {
            ...
            {
                message = $"{message} (HRESULT=0x{ex.HResult:X8})";
            }

            return Task.FromResult(("failed", 1, (object?)new
            {
                error = "screen capture failed",
                stderr = SnapshotJobSupport.Truncate(message, 1200),
                remote_support = new
                {
                    session_id = sessionId,
                },
    ...
    private static int ReadInt(Dictionary<string, object?> payload, string key, int fallback, int min, int max)
    {
        if (payload.TryGetValue(key, out var value) && value is not null)
        {
            if (value is JsonElement json)
            {
                if (json.ValueKind == JsonValueKind.Number && json.TryGetInt32(out int numeric))
                {
                    return Math.Clamp(numeric, min, max);
                }
                if (json.ValueKind == JsonValueKind.String && int.TryParse(json.GetString(), out numeric))
                {
                    return Math.Clamp(numeric, min, max);
                }
            }
    ...
    private static (string Base64, int ByteSize) CaptureAsJpegBase64(int maxDimension, int jpegQuality)
    {
        byte[] bytes = CaptureAsJpegBytesWithFallback(maxDimension, jpegQuality);
        return (Convert.ToBase64String(bytes), bytes.Length);
    }

    internal static byte[] CaptureAsJpegBytesWithFallback(int maxDimension, int jpegQuality)
    {
        int safeMaxDimension = Math.Clamp(maxDimension, 640, 3840);
        int safeQuality = Math.Clamp(jpegQuality, 25, 95);
        try
        {
            return CaptureAsJpegBytesFromCurrentDesktop(safeMaxDimension, safeQuality);
        }
        catch (Exception primaryError)
        {
            return CaptureAsJpegBytesViaInteractiveTask(safeMaxDimension, safeQuality, primaryError);
        }
    }

    internal static byte[] CaptureAsJpegBytesFromCurrentDesktop(int maxDimension, int jpegQuality)
    {
        int safeMaxDimension = Math.Clamp(maxDimension, 640, 3840);
        int safeQuality = Math.Clamp(jpegQuality, 25, 95);
        Rectangle virtualBounds = GetVirtualScreenBounds();
        if (virtualBounds.Width < 1 || virtualBounds.Height < 1)
        {
            throw new InvalidOperationException("virtual screen not available");
        }

        using var source = new Bitmap(virtualBounds.Width, virtualBounds.Height, PixelFormat.Format24bppRgb);
        using (var graphics = Graphics.FromImage(source))
        {
            graphics.CopyFromScreen(virtualBounds.Left, virtualBounds.Top, 0, 0, source.Size, CopyPixelOperation.SourceCopy);
        }

        using var output = ResizeIfNeeded(source, safeMaxDimension);
        using var stream = new MemoryStream();
        SaveAsJpeg(output, stream, safeQuality);
        byte[] bytes = stream.ToArray();
        if (bytes.Length == 0)
        {
            throw new InvalidOperationException("capture output is empty");
        }

        return bytes;
    }

    private static byte[] CaptureAsJpegBytesViaInteractiveTask(int maxDimension, int jpegQuality, Exception primaryError)
    {
        if (!RuntimeInformation.IsOSPlatform(OSPlatform.Windows))
        {
        ...
        if (string.IsNullOrWhiteSpace(processPath) || !File.Exists(processPath))
        {
            throw new InvalidOperationException(
                $"screen capture failed in service session and helper executable was not found: {primaryError.Message}",
                primaryError);
        }
        ...
        string programData = Environment.GetEnvironmentVariable("ProgramData")
            ?? Environment.GetFolderPath(Environment.SpecialFolder.CommonApplicationData)
            ?? @"C:\ProgramData";
        string root = Path.Combine(programData, "DMS", "RemoteSupport");
        Directory.CreateDirectory(root);

        string token = Guid.NewGuid().ToString("N");
        string taskName = $"DMS-RemoteCapture-{token}";
        string outputPath = Path.Combine(root, $"{token}.jpg");
        string errorPath = Path.Combine(root, $"{token}.err.txt");
        string launcherPath = Path.Combine(root, $"{token}.vbs");

        string helperCommand = $"\"{processPath}\" --capture-helper --out \"{outputPath}\" --error \"{errorPath}\" --max-dimension {maxDimension} --jpeg-quality {jpegQuality}";
        string helperCommandVbs = helperCommand.Replace("\"", "\"\"", StringComparison.Ordinal);
        string launcherScript = string.Join(Environment.NewLine, new[]
        {
            "Set shell = CreateObject(\"WScript.Shell\")",
            $"code = shell.Run(\"{helperCommandVbs}\", 0, True)",
            "WScript.Quit code",
        });
        File.WriteAllText(launcherPath, launcherScript, Encoding.ASCII);

        string taskRunCommand = $"wscript.exe //B //Nologo \"{launcherPath}\"";
        string escapedTaskRunCommand = ProcessRunner.EscapeForSchtasksTaskRun(taskRunCommand);

        try
        {
            var create = ProcessRunner.RunAsync(
                "schtasks",
                $"/Create /TN \"{taskName}\" /SC ONCE /ST 00:00 /RU \"INTERACTIVE\" /IT /TR \"{escapedTaskRunCommand}\" /F",
                CancellationToken.None).GetAwaiter().GetResult();
            if (create.ExitCode != 0)
            {
                string detail = ReadFirstNonEmpty(create.StdErr, create.StdOut);
                throw new InvalidOperationException(
                    $"screen capture failed: helper task create failed (exit {create.ExitCode}): {detail} | primary: {primaryError.Message}",
                    primaryError);
            }

            var run = ProcessRunner.RunAsync("schtasks", $"/Run /TN \"{taskName}\"", CancellationToken.None).GetAwaiter().GetResult();
            if (run.ExitCode != 0)
            {
                string detail = ReadFirstNonEmpty(run.StdErr, run.StdOut);
                throw new InvalidOperationException(
                    $"screen capture failed: helper task run failed (exit {run.ExitCode}): {detail} | primary: {primaryError.Message}",
                    primaryError);
            }

            DateTime deadlineUtc = DateTime.UtcNow.AddSeconds(InteractiveHelperTimeoutSeconds);
            while (DateTime.UtcNow < deadlineUtc)
            {
                if (File.Exists(outputPath))
                {
                    ...
                    if (bytes.Length == 0)
                    {
                        throw new InvalidOperationException($"screen capture failed: helper output is empty | primary: {primaryError.Message}", primaryError);
                    }
                    return bytes;
                }
                ...
                if (File.Exists(errorPath))
                {
                    ...
                    {
                        helperError = "helper reported a failure without details";
                    }

                    throw new InvalidOperationException(
                        $"screen capture failed: {helperError} | primary: {primaryError.Message}",
                        primaryError);
                }
            ...
            throw new InvalidOperationException(
                $"screen capture failed: helper timed out after {InteractiveHelperTimeoutSeconds} seconds | primary: {primaryError.Message}",
                primaryError);
        }
        finally
        {
            ...
            TryDeleteRemoteCaptureFile(outputPath);
            TryDeleteRemoteCaptureFile(errorPath);
            TryDeleteRemoteCaptureFile(launcherPath);
        }
    }
    ...
    private static void SaveAsJpeg(Bitmap image, Stream destination, int jpegQuality)
    {
        int safeQuality = Math.Clamp(jpegQuality, 25, 95);
        ImageCodecInfo? codec = ImageCodecInfo.GetImageEncoders()
            .FirstOrDefault(x => string.Equals(x.MimeType, "image/jpeg", StringComparison.OrdinalIgnoreCase));
        if (codec is null)
        {
    ...
    private static void TryDeleteRemoteCaptureFile(string path)
    {
        try
        {
            if (File.Exists(path))
...
public sealed class LiveScreenStreamHandler : IJobHandler
{
    private readonly ApiClient _apiClient;

    public LiveScreenStreamHandler(ApiClient apiClient)
    {
        _apiClient = apiClient;
    }

    public string JobType => "start_live_stream";

    public async Task<(string Status, int ExitCode, object? Result)> ExecuteAsync(CommandEnvelopeDto envelope, CancellationToken cancellationToken)
    {
        if (!RuntimeInformation.IsOSPlatform(OSPlatform.Windows))
        {
            return ("failed", 1, new { error = "start_live_stream is currently supported on Windows endpoints only" });
        }

        int maxDimension = ReadInt(envelope.Payload, "max_dimension", 1366, 640, 3840);
        int jpegQuality = ReadInt(envelope.Payload, "jpeg_quality", 60, 25, 95);
        int fps = ReadInt(envelope.Payload, "fps", 8, 1, 20);
        int durationSeconds = ReadInt(envelope.Payload, "duration_seconds", 120, 20, 600);
        string sessionId = envelope.Payload.TryGetValue("session_id", out var sessionObj) ? sessionObj?.ToString() ?? string.Empty : string.Empty;
        if (string.IsNullOrWhiteSpace(sessionId))
        {
            return ("failed", 1, new { error = "session_id missing" });
        }

        int frameIntervalMs = Math.Max(50, (int)Math.Round(1000d / Math.Max(1, fps)));
        ...
        {
            DateTime frameStartUtc = DateTime.UtcNow;
            try
            {
                byte[] frameBytes = ScreenCaptureHandler.CaptureAsJpegBytesWithFallback(maxDimension, jpegQuality);
                await _apiClient.PostRemoteSupportLiveFrameAsync(
                    sessionId,
            ...
            int processingMs = (int)Math.Max(0, (DateTime.UtcNow - frameStartUtc).TotalMilliseconds);
            int delayMs = Math.Max(0, frameIntervalMs - processingMs);
            if (delayMs > 0)
            {
                await Task.Delay(Math.Min(delayMs, remainingMs), cancellationToken);
            }
        }

        if (sentFrames == 0)
        {
            string err = string.IsNullOrWhiteSpace(lastError) ? "no live frame sent" : lastError;
            return ("failed", 1, new
            {
                error = "live stream failed",
                stderr = SnapshotJobSupport.Truncate(err, 1200),
                remote_support = new
                {
                    session_id = sessionId,
                    mode = "live",
...
public sealed class WebRtcSessionHandler : IJobHandler
{
    private readonly ApiClient _apiClient;
    private const int InteractiveHelperGraceSeconds = 30;

    public WebRtcSessionHandler(ApiClient apiClient)
    {
        _apiClient = apiClient;
    }

    public string JobType => "start_webrtc_session";

    public async Task<(string Status, int ExitCode, object? Result)> ExecuteAsync(CommandEnvelopeDto envelope, CancellationToken cancellationToken)
    {
        if (!RuntimeInformation.IsOSPlatform(OSPlatform.Windows))
        {
            return ("failed", 1, new { error = "start_webrtc_session is currently supported on Windows endpoints only" });
        }

        if (!WebRtcMediaPipelineCapability.IsBuildImplemented())
        {
            return ("failed", 1, new
            {
                error = ErrorCodes.Unsupported,
                message = "WebRTC media pipeline is not implemented on this endpoint build.",
            });
        }

        if (!WebRtcMediaPipelineCapability.IsConfiguredEnabled())
        {
            return ("failed", 1, new
            {
                error = ErrorCodes.Unsupported,
                message = "WebRTC media pipeline is disabled on this endpoint build.",
            });
        }
        ...
        int durationSeconds = ReadInt(envelope.Payload, "duration_seconds", 180, 20, 900);
        int maxDimension = ReadInt(envelope.Payload, "max_dimension", 1920, 640, 3840);
        List<WebRtcIceServerConfig> iceServers = ReadIceServers(envelope.Payload);

        return await RunInteractiveMediaHelperAsync(sessionId, durationSeconds, maxDimension, iceServers, cancellationToken);
    }

    private static async Task<(string Status, int ExitCode, object? Result)> RunInteractiveMediaHelperAsync(
        string sessionId,
        int durationSeconds,
        int maxDimension,
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (string.IsNullOrWhiteSpace(processPath) || !File.Exists(processPath))
        {
            return ("failed", 1, new { error = "webrtc helper executable not found" });
        }
        ...
        string token = Guid.NewGuid().ToString("N");
        string taskName = $"DMS-WebRtc-{token}";
        string configPath = Path.Combine(root, $"{token}.webrtc.json");
        string outputPath = Path.Combine(root, $"{token}.out.json");
        string errorPath = Path.Combine(root, $"{token}.err.txt");
        string launcherPath = Path.Combine(root, $"{token}.vbs");

        string configJson = JsonSerializer.Serialize(new WebRtcHelperConfig
        {
            SessionId = sessionId,
        ...
        string helperCommand = $"\"{processPath}\" --webrtc-helper --config \"{configPath}\" --out \"{outputPath}\" --error \"{errorPath}\"";
        string helperCommandVbs = helperCommand.Replace("\"", "\"\"", StringComparison.Ordinal);
        string launcherScript = string.Join(Environment.NewLine, new[]
        {
            "Set shell = CreateObject(\"WScript.Shell\")",
        ...
        try
        {
            ...
            if (create.ExitCode != 0)
            {
                return ("failed", 1, new
                {
                    error = "webrtc helper task create failed",
                    stderr = ReadFirstNonEmpty(create.StdErr, create.StdOut),
                });
            }
            ...
            if (run.ExitCode != 0)
            {
                return ("failed", 1, new
                {
                    error = "webrtc helper task run failed",
                    stderr = ReadFirstNonEmpty(run.StdErr, run.StdOut),
                });
            }
    ...
    {
        if (!payload.TryGetValue("ice_servers", out object? raw) || raw is null)
        {
            return [];
        }
        ...
        try
        {
            if (raw is JsonElement json && json.ValueKind != JsonValueKind.Undefined && json.ValueKind != JsonValueKind.Null)
            {
                return JsonSerializer.Deserialize<List<WebRtcIceServerConfig>>(json.GetRawText()) ?? [];
            }

            return JsonSerializer.Deserialize<List<WebRtcIceServerConfig>>(JsonSerializer.Serialize(raw)) ?? [];
        }
        catch
        {
            return [];
    ...
    private static void TryDeleteHelperFile(string path)
    {
        try
        {
            if (File.Exists(path))
...
public static class ScreenCaptureHelperCli
{
    public const int NotHandledExitCode = int.MinValue;
    ...
    public static int TryRun(string[] args)
    {
        ...
        try
        {
            ...
            {
                throw new InvalidOperationException("capture helper requires --out <path>");
            }

            errorPath = ReadOption(options, "error");
            int maxDimension = ReadIntOption(options, "max-dimension", 1366, 640, 3840);
            int jpegQuality = ReadIntOption(options, "jpeg-quality", 60, 25, 95);
            ...
            byte[] bytes = ScreenCaptureHandler.CaptureAsJpegBytesFromCurrentDesktop(maxDimension, jpegQuality);
        ...
        catch (Exception ex)
        {
            ...
            TryWriteError(errorPath, SnapshotJobSupport.Truncate(message, 1500));
            return 1;
        }
    }
    ...
    private static int ReadIntOption(Dictionary<string, string> options, string key, int fallback, int min, int max)
    {
        string raw = ReadOption(options, key);
    ...
    private static void TryWriteError(string path, string message)
    {
        ...
        try
        {
            string? errorDir = Path.GetDirectoryName(path);
