...
if [[ $# -lt 1 ]]; then
  ...
  echo "  AGENT_DIR=<path> (optional host path for DMS agent source repo)"
  echo "  RUN_SEEDERS=1|0 (default: 1)"
  exit 1
fi
...
AGENT_BACKEND_HOST="$AGENT_BACKEND_HOST" AGENT_BACKEND_PORT="$AGENT_BACKEND_PORT" AGENT_DIR="$AGENT_DIR" RUN_SEEDERS="$RUN_SEEDERS" \
